...
export default function AddMenu({ menu, store }: { menu: Menu; store: Store; }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  useEffect(() => {
    if (menu) {
      ...
      setValue('readyTime', Number(menu.readyTime));
      setGenre(menu.genre);
    }
  }, [menu]);
    ...
    <OwnerLayout header={<Header />}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className='flex-col p-4'>
          <div className='flex'>
            ...
            <div className='flex-col p-2'>
              <div className='flex gap-8'>
                ...
                <div className='flex-col'>
                  <p className='text-xl font-bold text-white mb-2'>準備時間</p>
                  <input
                    type='number'
          ...
          <div className='flex-col'>
            ...
            <Select
              options={genres}
              className='w-60'
              defaultValue={{ value: menu.genre, label: menu.genre }}
        ...
        <div className='flex flex-row items-center justify-between gap-4'>
          <Link
            href='/owner/menu'
            className='text-center my-8 w-full px-4 py-2 bg-white text-black font-bold rounded-lg duration-150 hover:opacity-80'
          >
            キャンセル
          </Link>
          <button
...
export async function getServerSideProps(ctx: GetServerSidePropsContext) {
  ...
  const menuList: { data: Menu[]; status: string; } = await menuData.json();
  const menu = menuList.data.filter(
    menu => menu.id === ctx.query.menuId!.toString(),
  );
  if (!menu) {
    return {
      redirect: {
