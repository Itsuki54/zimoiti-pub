  ...
  const handleSignUp = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/user', {
        ...
        body: JSON.stringify({ email, password, type: 'owner' }),
      });
      ...
      if (data.error) {
        ...
        router.push('/owner/signin?redirected=true');
      }
    }
    catch (error) {
    ...
    <OwnerLayout header={<Header />}>
      ...
      >
        ...
        <p className='mt-4 text-center'>
          <Link href='/owner/signin' className='text-primary'>
            ログインはこちら
          </Link>
        </p>
      </form>
